...
(function($) {
    ...
    var keys = ["hourly", "daily", "weekly", "monthly", "yearly"];
    for (var i = 0; i < periods.length; i++) {
        str_opt_period += "<option value='"+periods[i]+"'>" + keys[i] + "</option>\n";
    }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["period"] = $("<span class='cron-period'>"
                    + "<span class='glyphicon glyphicon-time'></span> Fetch <select name='cron-period'>" + custom_periods
